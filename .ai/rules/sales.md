...
## CreateAction::before() no tiene $record todavía — usar array $data
En un `CreateAction::make()->before(function (...) {...})` (como el de `ListSales::getHeaderActions()`), el modelo aún NO existe — inyectar `Sale $record` ahí tira `TypeError: ... must be of type App\Models\Sale, null given`. `before()` corre después de validar el form pero antes de instanciar/guardar el registro; para leer datos del form en ese punto hay que inyectar `array $data` (los datos ya validados), no `$record`. `$record` recién está disponible en `->after(function (Sale $record) {...})`, que es donde ya se usa para `deducirStock()`.

Este es el hook usado para bloquear la creación cuando `total <= 0` (`$action->halt()` + `Notification::make()->danger()->send()`), verificado con `tests/Feature/SaleCreateValidationTest.php`.
